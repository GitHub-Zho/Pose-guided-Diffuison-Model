...
# set the prompts and the negative prompts 
...
# add the control net model to base model
controlnet = ControlNetModel.from_pretrained(controlnet_path, torch_dtype=torch.float16)


# set the generator to generator the image at the very same situation (will generate the exact same image)
generator = torch.Generator("cuda").manual_seed(31)

# load the pipeline and initial some setting
...
# add the lora weights to the model
# pipeline.load_lora_weights(lora_path, weight_name=lora_name)

# add the CrossFrame processor to the model
...
for k in range(repeated_times):
    # extract the latent spaces and add the latent to other iterations
